...
bool Device::is_alive() {
  ...
  if (!read_usbtingo_serial(serial))
    return false;
...
bool Device::start_logic_stream(std::uint32_t samplerate_hz) {
  ...
  std::uint8_t prescaler = static_cast<std::uint8_t>(std::clamp(
...
bool Device::read_status(StatusFrame& status) {
  ...
  if (!read_control(USBTINGO_CMD_GET_STATUSREPORT, 0, 0, status_buffer, static_cast<uint16_t>(status_buffer.size())))
    return false;
...
bool Device::read_usbtingo_info() {
  ...
  if (!read_control(USBTINGO_CMD_GET_DEVICEINFO, 0, 0, data, 12))
    return false;
  ...
  m_device_info.unique_id = serialize_uint32(data.at(4), data.at(5), data.at(6), data.at(7));
