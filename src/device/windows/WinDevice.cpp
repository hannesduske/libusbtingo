...
bool WinDevice::send_can(const device::CanTxFrame& tx_frame) {
  ...
  if (!CanTxFrame::serialize_can_frame(tx_buffer.data(), tx_frame))
    return false;
...
bool WinDevice::send_can(const std::vector<device::CanTxFrame>& tx_frames) {
  ...
  for (const auto& tx_frame : tx_frames) {
    ...
    if ((msg_size + current_msg_size) > USB_BULK_BUFFER_SIZE) {
      if (!write_bulk(USBTINGO_EP3_CANMSG_OUT, tx_buffer, msg_size))
        return false;
      msg_size = 0;
    }

    // Add next message to the buffer
    if (!CanTxFrame::serialize_can_frame(tx_buffer.data() + msg_size, tx_frame))
      return false;
...
bool WinDevice::receive_can(std::vector<device::CanRxFrame>& rx_frames, std::vector<device::TxEventFrame>& tx_event_frames) {
  ...
  if (!read_bulk(USBTINGO_EP3_CANMSG_IN, rx_buffer, rx_len))
    return false;
...
bool WinDevice::receive_can_async(std::vector<CanRxFrame>& rx_frames, std::vector<TxEventFrame>& tx_event_frames) {
  ...
  if (!read_bulk_async(m_device_data, rx_len, m_async_can))
    return false;
...
bool WinDevice::receive_logic_async(LogicFrame& logic_frame) {
  ...
  std::size_t rx_len = 0;
  // synchronous operation, requires data to be available otherwise returns false
  if (!read_bulk_async(m_device_data, rx_len, m_async_logic))
    return false;
...
bool WinDevice::receive_status_async(StatusFrame& status_frame) {
  ...
  std::size_t rx_len = 0;
  // synchronous operation, requires data to be available otherwise returns false
  if (!read_bulk_async(m_device_data, rx_len, m_async_status))
    return false;
...
bool WinDevice::read_usbtingo_serial(std::uint32_t& serial) {
  ...
  bResult = WinUsb_GetDescriptor(m_device_data.WinusbHandle, USB_DEVICE_DESCRIPTOR_TYPE, 0, 0, (PBYTE)&deviceDesc, sizeof(deviceDesc), &lengthReceived);
...
HRESULT WinDevice::read_usb_descriptor(const WinHandle& device_data, const std::uint8_t index, const std::uint16_t languageID, std::string& value) {
  ...
  bResult = WinUsb_GetDescriptor(device_data.WinusbHandle, USB_STRING_DESCRIPTOR_TYPE, index, languageID, (PBYTE)buffer, bufferLength, &lengthTransferred);
...
HRESULT WinDevice::open_usb_device(WinHandle& device_data) {
  ...
  device_data.DeviceHandle = CreateFile(device_data.DevicePath.c_str(), GENERIC_WRITE | GENERIC_READ, FILE_SHARE_WRITE | FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL);
  ...
  bResult = WinUsb_Initialize(device_data.DeviceHandle, &device_data.WinusbHandle);
