...
/**
 * @brief Minimal example of a command line program that lists the serial numbers of all connected USBtingo devices. At the start all currently connected USBtingo serial numbers are printed. Subsequently, all connection and disconnection
 * events of USBtingo devices are printed.
 */
int main(int argc, char* argv[]) {
  ...
  std::cout << " This application detects and lists all" << std::endl << " connected USBtingo devices." << std::endl << std::endl;
  ...
  if (serial_vec.size() > 0) {
    std::cout << " Currently connected devices:" << std::endl << std::endl;
    std::size_t idx = 0;
    ...
    std::cout << " There are no USBtingo devices currently" << std::endl << " connected." << std::endl;
  }
  ...
  std::cout << "+======================================+" << std::endl;
  std::cout << " Watch for newly connected devices:" << std::endl << " (Press ENTER to exit)" << std::endl << std::endl;
  ...
  while (!shutdown.load()) {
    ...
    for (const auto& serial : serial_vec) {
      if (std::none_of(current_vec.begin(), current_vec.end(), [&serial](std::uint32_t x) {
            return serial == x;
          })) {
    ...
    for (const auto& serial : current_vec) {
      if (std::none_of(serial_vec.begin(), serial_vec.end(), [&serial](std::uint32_t x) {
            return serial == x;
          })) {
